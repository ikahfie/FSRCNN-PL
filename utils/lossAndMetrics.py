...
def __distance(y_true, y_pred, loss_model):
    ...
    :param y_pred: predicted label class
    :param loss_model: VGG loss network
    :return: float of mean of abs distance between loss network features.
    """
    input0 = loss_model(y_true)
    input1 = loss_model(y_pred)
...
def perceptual_loss(alpha: float, vgg_layer_nums: list, tiled: bool):
    ...
    def loss(y_true, y_pred):
        """
        Calculate perceptual loss based on batch y_true and y_pred.
        - Preproc-ing both y_true and y_pred with
          tf.keras.applications.vgg16.preprocess_input()
        - Calculating mean abs distance of each baches for each lossmodel.
        - PL.LOSS = Alpha * sum(loss_each_models) + (mae(y_true,y_pred).

        :param y_true: batch of ground truth images
        :param y_pred: batch of predicted images
        :return: float of perceptual loss per batch
        """
        y_true = preprocess_input(y_true)
        y_pred = preprocess_input(y_pred)
